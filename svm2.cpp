...
void Solver::Solve(int l, const QMatrix& Q, const double *p_, const schar *y_,
		   ...
		   SolutionInfo* si, int shrinking)
{
    qInfo() << "Solving using the solver instance." << Qt::endl;
	this->l = l;
	this->Q = &Q;
	QD=Q.get_QD();
	clone(p, p_,l);
...
{
    qInfo() << "Solving using solve_c_svc." << Qt::endl;
	int l = prob->l;
...
static decision_function svm_train_one(
	...
	double Cp, double Cn)
{
    qInfo() << "Training a single model" << Qt::endl;
...
static void svm_group_classes(const svm_problem *prob, int *nr_class_ret, int **label_ret, int **start_ret, int **count_ret, int *perm)
{
	...
	//max number of classes is initially set to 16
	int max_nr_class = 16;

	//initially the number of classes is 0
	int nr_class = 0;

	//set of unique labels- an int for each class in order 0, 1, 2... i.e. label[0] == 0, label[1] == 1, etc.
	int *label = Malloc(int,max_nr_class);

	// pointer to the number of observations of each class label
	int *count = Malloc(int,max_nr_class);

	//the data_labels for the entire set
	int *data_label = Malloc(int,l);

	//counter variable for going through each observation
	int i;

	//iterate through each observation in the set
	for(i=0;i<l;i++)
	{
		//get the label corresponding to this observation
		int this_label = (int)prob->y[i];

		//initialize the counter
		int j;

		//iterate through the classes discovered thus far
		for(j=0;j < nr_class; j++)
		{
			//if the current label is equal to j
			if(this_label == label[j]) //but label[j] has not been initialized?
			{
				//increment the number of observations that have that label
				++count[j];

				//break out of the loop since we know the label is j
				break;
			}
		}

		//assign the data_label for i to whichever value for j "this_label" was equal to 
		data_label[i] = j;

		//if j is a new class
		if(j == nr_class)
		{
			//if its equal to the max number of classes
			if(nr_class == max_nr_class)
			{
				//assign more memory for the number of classes
				max_nr_class *= 2;
			...
			//increment the count of the newly observed label
			count[nr_class] = 1;
	...
	if (nr_class == 2 && label[0] == -1 && label[1] == 1) //handling two-class problems with -1 and 1 labels
	{
    ...
    //class 0 data starts at index 0
	start[0] = 0;

    //initialize the starting indices for each of the classes
	...
	*label_ret = label;
    ...
    //set the start array to start
	*start_ret = start;

	//return the number of observations of each class
	*count_ret = count;
...
{
    qInfo() << "Beginning Training." << Qt::endl;

    //initialize the model which will be returned.
	...
	else
	{
        ...
        qInfo() << "Grouping the classes." << Qt::endl;

		svm_group_classes(prob,&nr_class,&label,&start,&count,perm);
		if(nr_class == 1)
			info("WARNING: training data in only one class. See README for details.\n");

        //declare an array for the training data
		svm_node **x = Malloc(svm_node *,l);

		int i;

        //reorder the data using the permutation
		for(i=0;i<l;i++)
			x[i] = prob->x[perm[i]];

		// calculate weighted C

        //initialize an array of size nr_classes of doubles
		double *weighted_C = Malloc(double, nr_class);

        // initialize each weight to C
        ...
        //an array of booleans of non-zero entries?
        ...
        //initialize an array of decision functions ?
		decision_function *f = Malloc(decision_function,nr_class*(nr_class-1)/2);

        //array of probabilities intially null
		...
		for(i=0;i<nr_class;i++)
            ...
            qInfo() << "Computing model: " << i << " " << Qt::endl;

            for(int j=i+1;j<nr_class;j++)
			{

                //initialize a sub-problem?
				svm_problem sub_prob;

                //index of start of class i
                int si = start[i];

                //index of start of class j
                int sj = start[j];
                ...
                //initialize the count of the sub-problem
				sub_prob.l = ci+cj;

                //array to hold the x features
                ...
                qInfo() << "Computing decision boundary between class " << i << "and class" << j << "." << Qt::endl;
        ...
        //set unique labels in the model
		for(i=0;i<nr_class;i++)
